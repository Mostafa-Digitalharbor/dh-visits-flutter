...
## 01 · Login — `screenshots/01-login-{light,dark}.png`

**Full‑bleed brand hero** (no rounded card — it bleeds to the screen edges, padding `16 22 70`):
- Background = the **Cairo map image** (`assets/map-cairo.png`, cover/center) under a **brand‑gradient
  veil**: `linear-gradient(155deg, navy‑900@92% 0%, brand@88% 55%, cyan‑600@82% 100%)`.
- Two soft radial **glow accents** (cyan top‑inline‑end 180px; white bottom‑inline‑start 150px).
- Top row: **glassy chips** (rgba‑white .12 bg, .22 border, blur 6) — a language chip
  (`translate` + "EN"/"ع") and a 34px circular theme‑toggle (`dark_mode`/`light_mode`). **No back button.**
- Logo lockup: **84px rounded‑24 white tile** (radius 24, soft drop + inset hairline) holding the 52px
  `logo-d.png`; below it app title **"Visits" 30/800 #fff** (letter‑spacing −0.5), tagline 13.5/600 white@82%.

**Credential sheet** — overlaps the hero by 44px (`margin‑top: -44`, radius‑top 28, padding `10 22 22`,
soft top shadow), opens with a 40×4 grabber:
- Title "أهلاً بعودتك" **23/800** (letter‑spacing −0.3) + sub "سجّل دخولك لبدء يومك الميداني" 14/500 secondary.
- **Segmented role toggle** — single pill track (`surface-container`, padding 5) with a **sliding white
  thumb** (elev‑1, 300ms emphasized ease) under the active half; two halves `مدير` `shield_person` /
  `موظف ميداني` `badge` (h44, icon 20 + label 14/700; active text = `brand`, idle = tertiary). Picking a
  role slides the thumb AND updates the username (`m.adel` / `mona.fouad`).
- Two `AppTextField` (person / lock+eye), prefilled.
- **Remember + Forgot row** — left: 20px rounded‑6 checkbox (filled `brand` + white `check` when on) +
  "تذكّرني" 13/600; right: "نسيت كلمة السر؟" 13/700 `brand` link.
- **Filled CTA** "تسجيل الدخول" `login`, full‑width, size lg.
- **Footer** (pushed to bottom) — centered `shield` 14 + "دخول آمن · Digital Harbor" 11.5/600 disabled.

`onLogin(role)` → shell. Hero veil + glow + map are pure decoration; only the logo and gradient are brand‑critical.

---

## 02 · Manager Dashboard — `screenshots/02-dashboard-{light,dark}.png`
...
## 03 · Analytics — `screenshots/04-analytics-{light,dark}.png`  (manager)
...
## 04 · Team Visits — `screenshots/03-team-visits-{light,dark}.png`  (manager)
...
## 05 · Visit Detail — MANAGER (monitoring) — `screenshots/06-visit-detail-manager-{light,dark}.png`
...
## 06 · Create Visit sheet — `screenshots/10-create-visit-{light,dark}.png`  (manager)
...
## 07 · Settings — `screenshots/14-settings-{light,dark}.png`  (both)
...
## 08 · Customers — `screenshots/12-customers-{light,dark}.png`  (manager)
...
## 09 · Review queue — `screenshots/11-review-{light,dark}.png`  (manager)
...
## 10 · My Visits — `screenshots/05-my-visits-{light,dark}.png`  (employee)
...
## 11 · Visit Detail — EMPLOYEE · scheduled — `screenshots/07-visit-scheduled-{light,dark}.png`

> The map header (geofence circle + animated GPS pulse rings + pins + radar sweep) is documented in
> full, with timings and `lib/geofence_map.dart`, in **`05-map-and-geofence.md`**.
...
## 12 · Visit Detail — EMPLOYEE · active — `screenshots/08-visit-active-{light,dark}.png`
...
## 13 · Report sheet — `screenshots/09-report-sheet-{light,dark}.png`  (employee)
...
## 14 · Today's Route — `screenshots/13-route-{light,dark}.png`  (employee)
...
## Dark theme — every screen ships a `-dark` variant

Every screenshot above has a matching `*-dark.png`. The dark scheme uses bg `#0A0B0F`, surfaces
`#14161C`/`#1B1E27`, text `#ECECF4`, dividers/outlines at low‑opacity white, and tone‑containers shift
to deep tints (see `01-foundations.md` for the full dark hex column). The map on detail/route/dashboard
is tinted darker via the colour matrix in `lib/geofence_map.dart`; pins, veil, rings and all chrome
stay identical. Nothing is light‑only — build each widget reading `Theme.of(context).colorScheme` and
the `AppX` extension, never hard‑coded colours, and both themes fall out automatically.
